...
class UNet(nn.Module):
    def __init__(self):
        ...
        self.enc1 = self.conv_block(3, 64, kernel_size=2, padding='same')
        self.enc2 = self.conv_block(64, 128, kernel_size=2, padding='same')
        self.enc3 = self.conv_block(128, 256, kernel_size=2, padding='same')
        self.enc4 = self.conv_block(256, 512, kernel_size=2, padding='same')
        self.bottleneck = self.conv_block(512, 1024, kernel_size=2, padding='same')
        ...
        self.dec4 = self.conv_block(1024, 512, kernel_size=2, padding='same')
        ...
        self.dec3 = self.conv_block(512, 256, kernel_size=2, padding='same')
        ...
        self.dec2 = self.conv_block(256, 128, kernel_size=2, padding='same')
        ...
        self.dec1 = self.conv_block(128, 64, kernel_size=2, padding='same')
        # Couches de sortie
    ...
    def prediction(self,X):
        with torch.no_grad():
            return self.forward(torch.from_numpy(X.reshape(1,3, size, size).astype(np.float32))).detach().numpy().reshape(size,size)
